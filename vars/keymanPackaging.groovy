...
def call(body) {
  ...
  ansiColor('xterm') {
    timestamps {
      ...
      // Don't remove the withEnv - although it seems unnecessary it is needed because this
      // sets the value for `tag` that gets persisted between builds.
      withEnv(["tag=${tagToBuild}"]) {
        def tier
        def repoSuffix

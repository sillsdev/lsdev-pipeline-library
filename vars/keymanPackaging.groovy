...
def call(body) {
  ...
  ansiColor('xterm') {
    timestamps {
      ...
      withEnv(["TAG_TO_BUILD=${tagToBuild}"]) {
        def tier
        def repoSuffix
        ...
        node('master') {
          stage('checkout source') {
            ...
            echo "TAG_TO_BUILD=${env.TAG_TO_BUILD}, tag=${env.tag}, force=${env.force}, branch=${env.branch}, project=${env.project}, buildCause=${currentBuild.getBuildCauses('hudson.model.Cause$UserIdCause')}"
            echo "build causes: ${currentBuild.getBuildCauses()}"

            sh 'git fetch -q origin -p --tags && git clean -dxf'

            if (env.TAG_TO_BUILD) {
              sh "git checkout ${env.TAG_TO_BUILD}"
            }
...
""",
            ...
            if (!gitHub.isPRBuild() && env.TAG_TO_BUILD) {
              node(binaryPackagerNode) {
                unstash name: 'packages'
                for (d in distributionsToPackage.tokenize()) {
                  for (a in arches.tokenize()) {
...
"""
          ...
          } else if (env.TAG_TO_BUILD) {
            currentBuild.description = "<span style='background-color: yellow'>${env.TAG_TO_BUILD}</span>"
          }
        } catch(Exception ex) {
          currentBuild.result = 'FAILURE'
          if (gitHub.isPRBuild()) {
